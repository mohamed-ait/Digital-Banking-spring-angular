package com.example.ebanckingbackend.security.filters;

import  com.auth0.jwt.JWT;
import com.auth0.jwt.JWTVerifier;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.interfaces.DecodedJWT;
import com.example.ebanckingbackend.security.config.JwtConfig;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if( request.getServletPath().equals( JwtConfig.REFRESH_PATH)){ // is it a refreshing ?
            filterChain.doFilter(request, response);
            return ;
        }
        String jwt_token = request.getHeader(JwtConfig.AUTHORIZATION_HEADER);
